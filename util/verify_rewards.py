...
def test_reward_logic():
    ...
    print("\n4. Test SQL Error (Override Intermediate)")
    # Must use EXEC_RESULT_DELIMITER to trigger error detection
    answer_with_error = f"Here is the query: ```sql SELECT * FROM Customer ```{EXEC_RESULT_DELIMITER}Error: syntax error at or near..."
    reward, _, tier = judge_correct(
    ...
    answer_permission = f"Here is the query: ```sql SELECT * FROM Customer ```{EXEC_RESULT_DELIMITER}Error: permission denied for relation customer"
    reward, _, tier = judge_correct(
